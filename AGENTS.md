...
- **V0.71.0** — 当前：IP-Adapter 视觉参考 + 文本分析增强<br>　　　　　　下载 XLabs Flux IP-Adapter 模型 (936MB, `ip-adapter-flux-xlabs.safetensors`)<br>　　　　　　`build_flux_workflow` 新增 `ref_image` / `ip_weight` 参数<br>　　　　　　ref_image 时插入 3 个 IP-Adapter 节点（LoadImage → IPAdapterUnifiedLoader → IPAdapterAdvanced）<br>　　　　　　`--ref` 参考图从纯文本分析升级为视觉条件控制（IP-Adapter + 文字双通道）<br>　　　　　　`_ollama_vl_analyze` 分析 prompt 改为英文 + 结构化 JSON（face/eye/hair/outfit/accessories/colors/background）<br>　　　　　　`ref_analyze_to_prompt` 返回新增 `composition/lighting/colors/background` 字段<br>　　　　　　`agents/go_flux.py` 工作流构建改造<br>　　　　　　`workshop/engine/engine.py` 参考图分析增强<br>　　　　　　`tests/test_go_flux_workflow.py` +11 项工作流测试（含 IP-Adapter 节点结构）<br>　　　　　　`tests/test_workshop_engine.py` +3 项增强字段测试<br>　　　　　　测试总数 283 → 297 项<br>　　　　　　版本 0.71.0<br>
