...
- **V0.58.0** — 当前：manga 重试 + create 排行榜<br>　　　　　　`workshop manga --retry N` 每格失败后重试（含递增延迟）<br>　　　　　　`generate_panels` 新增 `max_retries` 参数，空结果也触发重试<br>　　　　　　`workshop create` 输出候选排行榜 🥇🥈🥉（综合分/质检/CLIP）<br>　　　　　　版本 0.58.0<br>
