...
- **V0.5.0** — 当前：LoRA 训练/批处理/IPAdapter/多角色/Flux.2 Klein 均已可用
- V0.6.0 — 下一功能版本（待定）
...
| 能力 | 入口 | 说明 |
|------|------|------|
| 一句话出图 | `run.py` | 自然语言 → Ollama 转写英文 tag → ComfyUI 提交 |
| 角色 LoRA 文生图 | `go_knives_lora.py` | SDXL/SD1.5 多角色（Knives / Caster）、批量、换装 |
| IPAdapter 锁脸 | `go_knives_ipadapter.py` | 参考图驱动面部一致性、权重可调 |
| 多角色同框 | `go_multi_char_lora.py` | 双 LoRA + FaceDetailer 修脸 |
| 批处理 | `go_knives_lora.py --count N` | 多张自动复制到草稿库 |
